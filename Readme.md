...
One important note, this mapper works with large collections, from 5000 and up to 25000 items.
## Rules
...
### Why is it soo slow?
The entry point = is small method called **Map**.
The main logic are placed in the **MapSingle** method where we can find the general computation recourse waster.
```csharp
...
This changes will not dramatically improve the performance of the specific method by improving the overall performance of the service be reducing pressure on GC.
After these improvements the method executions time will be drastically decreased.
```csharp
...
There is ways how to get more performance without diving in **unsafe** code and making the mapper _readonly_.
...
Another feature of structs is structure of array and array based collections.
...
This manipulation gives us around 6% boost over to the previous method.
```csharp
